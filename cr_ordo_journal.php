	...
	<div id='div_ordonnancement' style='width:100%;display:none' >
		...
		function afficher_ligne($pdo_result,$th){
			...
			while($r){$i=0;
				...
				$mnt_mois=0;
				do{
					echo "<table><tr><th colspan='12' ><caption class='caption-com'>Les Ordonnancements Commune: ".("$r[commune]")." </caption></th></tr></table>";
					$mnt_c=0;
					do{$mnst=0;
						echo "
						<table>
							<thead $no_display ><tr><th colspan='12' >Ordonnancements Service: $r[service] </th></tr></thead>
							<tbody $no_display ><tr>$th</tr>";
						do{	$i++;					
							// $mnt += $r["montant_acte"];
							$mnst += $r["montant_acte"];
							echo "<tr><td>$i</td><td>$r[adresse_assujetti] </td>
							...
							else {$same_com = $same_mon = false;}
						}while($r and $same_ser and $same_com and $same_mon);
						echo "
							</tbody>
							<tfoot>
			...
			return array($i,$mntt,$t_mnt_mois);
		}
		...
		$STR_DONUT = "";
		$STR_LINES = "";
		$th = "<th>N°</th><th>Ass..</th><th>Adrs.</th><th>NIF</th><th>Acte</th><th>Fréq.</th><th>N°NP.</th><th style='width:150px'>Date Ord/Date Dp.</th><th>Art.Bud</th><th>Mnt.(Fc)</th><th>Obs.</th><th>Mt.Bap(Fc)</th>";//<th>Serv.</th>
		$totaux = 0;
		// ordonnancement de la ville de lubumbashi et uniquement dans les centres
		$whre_order = " and co.commune='Centre' order by annee, mois, ser.service, ser.id ";
		// echo $req.$whre_order;
		if($pdo_result = Chado::$conx->query($req.$whre_order)){
			echo"<div class='partie'>";
				echo"<div class='partie-head'><h1>TOUS LES ORDONNANCEMENTS DES CENTRES</h1></div>";
				...
				echo "</div>";
				echo"<div class='partie-foot'><h1>TOTAL ORDONNANCEMENTS DES CENTRES <b>".chiffre($rep[1])."</b></h1></div>";
			echo "</div>";
			$totaux += $rep[1];
			$STR_DONUT = "{value:$rep[1]_c_f00_h_f00_l_CENTRES'},";
			$STR_LINES = "&amp;CENTRES=".implode(',',$rep[2]);
		}
		// ordonnancement de la ville de lubumbashi et uniquement dans les antenne
		$whre_order = " and co.commune not in ('Centre','CB.ORDO','CB.ET - CONT') order by annee, mois, co.id,ser.service,ser.id ";
		// echo $req.$whre_order;
		if($pdo_result = Chado::$conx->query($req.$whre_order)){
			echo"<div class='partie'>";
				echo"<div class='partie-head'><h1>TOUS LES ORDONNANCEMENTS DES ANTENNES</h1></div>";
				echo "<div class='partie-body'>";
					$rep = afficher_ligne($pdo_result,$th);
					if($rep[0]==0)echo "
					<table style='width:1300px'>
				...
				echo "</div>";
				echo"<div class='partie-foot'><h1>TOTAL ORDONNANCEMENTS DES ANTENNES <b>".chiffre($rep[1])."</b></h1></div>";
			echo "</div>";
			$totaux += $rep[1];
			$STR_DONUT .= "{value:$rep[1]_c_0f0_h_0f0_l_ANTENNES'},";
			$STR_LINES .= "&amp;ANTENNES=".implode(',',$rep[2]);
		}		
		// tous les ordonnancement confondus hors mis ceux de la ville de Lubumbashi 
		$whre_order = " and co.commune in ('CB.ORDO','CB.ET - CONT')order by annee, mois, ser.service,ser.id ";
		if($pdo_result = Chado::$conx->query($req.$whre_order)){ //echo $req.$whre_order;
			echo"<div class='partie'>";
				echo"<div class='partie-head'><h1>TOUS LES ORDONNANCEMENTS DES CB.ORDO ET CB.ETUDE</h1></div>";
				echo "<div class='partie-body'>";
					$rep = afficher_ligne($pdo_result,$th);
					if($rep[0]==0)echo "
					<table style='width:1300px'>
				...
				echo "</div>";
				echo"<div class='partie-foot'><h1>TOTAL ORDONNANCEMENTS DES BUREAUX ORDO. & ETUDS <b>".chiffre($rep[1])."</b></h1></div>";
			echo "</div>";
			$totaux += $rep[1];
			$STR_DONUT .= "{value:$rep[1]_c_00f_h_00f_l_BUREAUX'}";
			$STR_LINES .= "&amp;BUREAUX=".implode(',',$rep[2]);
		}
