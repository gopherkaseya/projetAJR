		...
		if($pdo_result = Chado::$conx->query($req.$whre_order)){//echo $req.$whre_order;
			...
			while($r = $pdo_result->fetch(PDO::FETCH_ASSOC)){
				...
				if(isset($tab[$month][$cl_lg][$cl_col]))
					$tab[$month][$cl_lg][$cl_col] += $r["montant_acte"];
				else {
				...
				$tab_col[$month][$cl_col] += $r["montant_acte"];
			}
			...
			foreach($tab as $month=>$tab_month){
				...
				foreach($tab_month as $cle=>$ligne){
					...
					for($i=0;$i!=count($tab_col_month);$i++){
						...
						if(isset($ligne[$tab_col_month[$i]])){
							$mt = $ligne[$tab_col_month[$i]];
							$total += $mt;
						}
						// if(isset($lg_tt[$tab_col_month[$i]]))$lg_tt[$tab_col_month[$i]] += $mt;
				...
				$tab_tot_an[] =$ttt;
				echo "<td style='font-weight:bold;'>".($ttt=chiffre($ttt))."</td></tr>";
				echo "</table>";
			...
			// $STR_DONUT = "{value:$rep[1]_c_f56954_h_f56954_l_CENTRES'},";
			// $STR_DONUT .= "{value:$rep[1]_c_00a65a_h_00a65a_l_ANTENNES'},";
			// $STR_DONUT .= "{value:$rep[1]_c_f39c12_h_f39c12_l_BUREAUX'}";
