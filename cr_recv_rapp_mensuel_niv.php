	...
	<?php
		...
		for($mois_d = 1; $mois_d<= $limite_mois; $mois_d++ ){
			...
			{$req = "SELECT n_ap.montant_payer ,ser.service ,ac.acte ,ac.art_bud ,nac.freq ,no.date_ordo ,no.date_depot ,ass.nom_assujetti ,ass.adresse_assujetti ,no.num_note ,no.num_bap ,ifnull('',concat(no.num_bap,' ',no.montant_bap))montant_bap ,no.note_to ,no.date_save date_enrg ,nac.ajouter_le date_enreg ,co.commune,co.id id_commune,extract(YEAR FROM no.date_depot)annee, extract(MONTH FROM no.date_depot)mois
			...
			WHERE rlv.is_deleted=0 and n_ap.is_deleted=0 and nac.is_deleted=0 and no.is_deleted=0 and rlv.is_deleted=0 
			...
			if($pdo_result = Chado::$conx->query($req.$whre_order)){//echo $req.$whre_order;
				...
				//
				$totaux = 0;
				echo "<table style='width:100%;font-weight:bold;font-style:italic;text-align:center'><caption style='font-weight:bold;font-size:16px;font-style:italic;color:#4e78c2'>RAPPORT MENSUEL DES RECOUVREMENTS NIVELES DE: $MOIS[$month]</caption>";
				...
				echo "<th style='font-style:italic;min-width: 100px;'>TOTAL</th></tr>";
				//
			...
			echo "<div class='div-mois-foot'>Total Ordonnancements <strong>$moiss</strong> <b>".chiffre($totaux)."</b></div>";
