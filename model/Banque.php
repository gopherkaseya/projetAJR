<?php
	...
	class Banque extends Chado
	{
		...
		public static $table 	= 't_banque';
		public static $marqueur = "<input type='hidden' name='class' value='Banque' >";
		public static $liste_modifiable= true;
		public static $useDetailStandard= true;
		public static $id_div_liste = 'id_div_liste_Banque';
		public static $id_div_crud = 'id_div_crud_Banque';
		public static $retourHtmlRapConfig = 'div_form_rapp_Banque';
		...
		public static function liste_objet($where,$limit){
			$ch_id = "bq.id";
			$Select = "SELECT $ch_id, bq.nom_banque,bq.adresse_banque,bq.telephone_banque FROM ".self::$table ." bq ";
			$req = "$Select  $where $limit";
			
			$count = "SELECT count(*) nbre FROM ".self::$table ." bq ";
		...
		function detail($id){
			...
			if($pdo_result = self::$conx->query($req)){
				...
				<table><caption style='".self::$style_caption."'><b>Détails sur Banque</b></caption>
					<tbody style='text-align:left'>
					<tr><th>Banque</th><td>:</td><td>$row[nom_banque]</td></tr>
					<tr><th>Adrss.Banque</th><td>:</td><td>$row[adresse_banque]</td></tr>
					<tr><th>Tél.Banque</th><td>:</td><td>$row[telephone_banque]</td></tr>
					</tbody>
				</table>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Banque ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
				...
				echo " <label class='".self::$bt_class."'  id='bt_termier_".self::$table."'  onClick=\"cache1_et_affiche2('".self::$id_div_crud."','".self::$retourHtmlAjaxLIST."');\" style='display:inline-block' >Retourner à la liste de Banque</label>
				</div></div>";
		...
		function detail_personnaliser($id){
			...
			if($pdo_result = self::$conx->query($req)){
				$row		= $pdo_result->fetch(PDO::FETCH_ASSOC);
				echo"<div id='".self::$id_div_crud."' style='' >
				<p><caption style='".self::$style_caption."'><b>Détails sur Banque</b></caption>Banque : $row[nom_banque] Adrss.Banque : $row[adresse_banque] Tél.Banque : $row[telephone_banque] 
				</p>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Banque ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
		...
		function form_rech_avancee(){
			...
			<form id='form_rechAc_".self::$table."' method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:none' > 
				...
				<input style='width:100px' type='text' name='bq_adresse_banque'  title=\"Adrss.Banque\" placeholder=\"Adrss.Banque\" class='".self::$input_class."'/>
				<input style='width:100px' type='text' name='bq_telephone_banque'  title=\"Tél.Banque\" placeholder=\"Tél.Banque\" class='".self::$input_class."'/>
		...
		function table_liste(){
			$Select = "SELECT bq.id ,bq.is_deleted ,bq.nom_banque,bq.adresse_banque,bq.telephone_banque ";
			$FROM = " FROM ".self::$table ." bq ";
			$Filtre = " WHERE 1=1 ";
			if(isset($_POST["Filtre"]) and "" != $_POST["Filtre"]){
				$w = $_POST["Filtre"];
				$Filtre .= " AND (1=2 or bq.nom_banque like '%$w%' or  bq.adresse_banque like '%$w%' or  bq.telephone_banque like '%$w%') ";
			}
			if(isset($_POST["bt_lancer_actAv_".self::$table])){
			$Filtre .= " AND (1=1 " 
			.(trim($t=$_POST["bq_nom_banque"])!=''?(" AND bq.nom_banque LIKE ".self::$conx->quote("%$t%")):"" )
			.(trim($t=$_POST["bq_adresse_banque"])!=''?(" AND bq.adresse_banque LIKE ".self::$conx->quote("%$t%")):"" )
			.(trim($t=$_POST["bq_telephone_banque"])!=''?(" AND bq.telephone_banque LIKE ".self::$conx->quote("%$t%")):"" )
			.")";
			}
			$rows= array(); $recordCount= 0;
			$FROM .= "  $Filtre ";
			$order = " order by bq.id DESC";
			...
			$taille = $_SESSION["Banque"]["t"] = (isset($_POST["taille_pg"])and $_POST["taille_pg"]!="")?$_POST["taille_pg"]:(isset($_SESSION["Banque"]["t"])?$_SESSION["Banque"]["t"]:10);
			...
			if($pdo_result = self::$conx->query($Select.$FROM.$order.self::$limit)){
				...
				<table style='margin-top:4px'>
					<caption style='".self::$style_caption."'><b>Liste de Banques</b></caption><tr><th></th><th>N°</th><th>Banque</th><th>Adrss.Banque</th><th>Tél.Banque</th>".
				...
				while($row		= $pdo_result->fetch(PDO::FETCH_ASSOC)){
					...
					echo "<td>".($i++)."</td><td>$row[nom_banque]</td><td>$row[adresse_banque]</td><td>$row[telephone_banque]</td>";
					eval($crud);
					echo "</tr>";
				}
				echo '</table>';			
				paginner(self::$count,$active_pg,$taille,'Banque',self::$id_div_liste,false);
				echo'</div>';
			}
			else if(self::$conx->errorInfo()[2]!="") {echo self::$notification .= self::$conx->errorInfo()[2];}
		}
		
		public static function insert($nom_banque,$adresse_banque,$telephone_banque){
			$id_visite = isset($_SESSION['sys_visites']['id'])?$_SESSION['sys_visites']['id']:0;
			$req = "INSERT INTO ".self::$table."(nom_banque,adresse_banque,telephone_banque,id_visite) VALUES(" .self::$conx->quote($nom_banque).','.self::$conx->quote($adresse_banque).','.self::$conx->quote($telephone_banque). ",'$id_visite');";
		...
		function creer() {
			if( true  and ''!=$_POST["nom_banque"] and ''!=$_POST["adresse_banque"] and ''!=$_POST["telephone_banque"]){
				if(self::insert($_POST["nom_banque"],$_POST["adresse_banque"],$_POST["telephone_banque"]))
					self::$notification .= self::$notifInsSucc." <form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' ><input type='submit'  name='bt_detail_".self::$table."' value='Détailler dernier(ère) Banque' class='".self::$bt_class."' /><input type='hidden' name='id' value='".self::$id."' />".self::$marqueur." </form> "; 
		...
		function modifier(){
			if(isset($_POST['id'])and (($id = $_POST['id'])!='')){
				if( true  and ''!=$_POST["nom_banque"] and ''!=$_POST["adresse_banque"] and ''!=$_POST["telephone_banque"]){
					$req = ("UPDATE ".self::$table." SET nom_banque = ".self::$conx->quote($_POST["nom_banque"])." ,adresse_banque = ".self::$conx->quote($_POST["adresse_banque"])." ,telephone_banque = ".self::$conx->quote($_POST["telephone_banque"])."  WHERE id = $id;");
		...
		//insertion dans la base de données
		function traitement() {
			if(isset($_POST['class'])and $_POST['class']!='Banque')return false;
		...
		function form($opt,$array){
				...
				$cle='nom_banque';$nom_banque=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='adresse_banque';$adresse_banque=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='telephone_banque';$telephone_banque=isset($array["$cle"])?$array["$cle"]:''; 
			...
			if($opt=="sup"  ){
				if(isset($_POST['id'])){
					...
					"<form id='' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='border:solid 0px;'>".
						"<table><caption style='".self::$style_caption."'><b>Suppression Banque</caption><tr><td >Voulez-vous réellement supprimer ? $ch_input<tr><td style='text-align:center'>
				...
				"<form enctype='multipart/form-data' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style=''>".
					"$ch_input".
					"<table>
						<caption style='".self::$style_caption."'><b>".($opt=='ajt'?'Enregistrement':'Modification')." Banque</caption>".
					"<tr>".
						"<td><label for='nom_banque' >Banque</label></td><td>:</td>".
						"<td><input type='text' value=\"$nom_banque\" title=\"Banque\" placeholder=\"Banque\" id='nom_banque' name='nom_banque' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>". 
					"<tr>".
						"<td><label for='adresse_banque' >Adrss.Banque</label></td><td>:</td>".
						"<td><input type='text' value=\"$adresse_banque\" title=\"Adrss.Banque\" placeholder=\"Adrss.Banque\" id='adresse_banque' name='adresse_banque' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>". 
					"<tr>".
						"<td><label for='telephone_banque' >Tél.Banque</label></td><td>:</td>".
						"<td><input type='text' value=\"$telephone_banque\" title=\"Tél.Banque\" placeholder=\"Tél.Banque\" id='telephone_banque' name='telephone_banque' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>".  
		...
		public static function options($where,$id) {
			...
			$req = "SELECT bq.id Value ,concat(bq.nom_banque) DisplayText FROM ".self::$table ." bq  WHERE bq.is_deleted=0 $where ";
			if($pdo_result = self::$conx->query($req)){
				$opt  ='';
				while($row = $pdo_result->fetch(PDO::FETCH_ASSOC)){
		...
		public static function ini_champ(){
			$T_CH = self::$T_CH = array("nom_banque","adresse_banque","telephone_banque");
			$l=0;$ch_alias="c";$label="l";$type="t";
			self::$TLBL = array(
			$T_CH[$l++]=>array($ch_alias=>"bq.nom_banque nom_banque",$label=>"Banque",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"bq.adresse_banque adresse_banque",$label=>"Adrss.Banque",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"bq.telephone_banque telephone_banque",$label=>"Tél.Banque",$type=>"text"));
		...
		public static function bt_rapport($class,$val){
			if(!self::$readOnly){echo "
				<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
					<input type='submit' class='$class' id='bt_rapport_complet_".self::$table."' name='bt_rapport_complet_".self::$table."' value='".($val!=''?$val:'Rapport Banque')."' onClick=\"".self::codeOnCallRapport()."\"/>
					".self::$marqueur."
				</form>";
			}
		}
		public static function rapport() {
			...
			$req = "SELECT bq.id $CONSTR[Ch_Select] FROM ".self::$table ." bq  ";
			self::suite_rapport($req,$CONSTR,self::$table,self::$marqueur,self::$style_caption,3);
		}
		...
		public static function form_rech_rapport(){
			...
			$T_SGO = self::$T_SGO;
			echo"
			<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."'>".(!(isset(self::$CHAMPS['nom_banque'][$T_SGO[3]])and self::$CHAMPS['nom_banque'][$T_SGO[3]])?"":"<input type='text' name='bq_nom_banque' title=\"Banque\" placeholder=\"Banque\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['adresse_banque'][$T_SGO[3]])and self::$CHAMPS['adresse_banque'][$T_SGO[3]])?"":"<input type='text' name='bq_adresse_banque' title=\"Adrss.Banque\" placeholder=\"Adrss.Banque\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['telephone_banque'][$T_SGO[3]])and self::$CHAMPS['telephone_banque'][$T_SGO[3]])?"":"<input type='text' name='bq_telephone_banque' title=\"Tél.Banque\" placeholder=\"Tél.Banque\" class='".self::$input_class."' />")
		...
		public static function bt_rapport_config($class,$val){
			echo "
			<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
				<input type='submit' class='$class' id='bt_rapport_config_".self::$table."' name='bt_rapport_config_".self::$table."' value='".($val!=''?$val:'Rapport Config Banque')."' onClick=\"".self::codeOnCallConfig()."\"/>
				".self::$marqueur."
			</form>";
		}		
