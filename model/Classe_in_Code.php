<?php
	...
	class Classe_in_Code extends Chado
	{
		public static $id;
		...
		public static $table 	= 'sys_classe_in_code';
		public static $marqueur = "<input type='hidden' name='class' value='Classe_in_Code' >";
		public static $liste_modifiable= true;
		public static $useDetailStandard= true;
		public static $id_div_liste = 'id_div_liste_Classe_in_Code';
		public static $id_div_crud = 'id_div_crud_Classe_in_Code';
		public static $retourHtmlRapConfig = 'div_form_rapp_Classe_in_Code';
		...
		public static function liste_objet($where,$limit){
			$ch_id = "sys_classe_in_code.id";
			$Select = "SELECT $ch_id, sys_classe_in_code.class_nom FROM ".self::$table ." sys_classe_in_code ";
			$req = "$Select  $where $limit";
			
			$count = "SELECT count(*) nbre FROM ".self::$table ." sys_classe_in_code ";
		...
		function detail($id){
			...
			if($pdo_result = self::$conx->query($req)){
				...
				<table><caption style='".self::$style_caption."'><b>Détails sur Classe_in_Code</b></caption>
					<tbody style='text-align:left'>
					<tr><th>Classe In Code</th><td>:</td><td>$row[class_nom]</td></tr>
					</tbody>
				</table>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Classe_in_Code ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
				...
				echo " <label class='".self::$bt_class."'  id='bt_termier_".self::$table."'  onClick=\"cache1_et_affiche2('".self::$id_div_crud."','".self::$retourHtmlAjaxLIST."');\" style='display:inline-block' >Retourner à la liste de Classe_in_Code</label>
				</div></div>";
		...
		function detail_personnaliser($id){
			...
			if($pdo_result = self::$conx->query($req)){
				$row		= $pdo_result->fetch(PDO::FETCH_ASSOC);
				echo"<div id='".self::$id_div_crud."' style='' >
				<p><caption style='".self::$style_caption."'><b>Détails sur Classe_in_Code</b></caption>Classe In Code : $row[class_nom] 
				</p>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Classe_in_Code ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
		...
		function table_liste(){
			$Select = "SELECT sys_classe_in_code.id ,sys_classe_in_code.is_deleted ,sys_classe_in_code.class_nom ";
			$FROM = " FROM ".self::$table ." sys_classe_in_code ";
			$Filtre = " WHERE 1=1 ";
			if(isset($_POST["Filtre"]) and "" != $_POST["Filtre"]){
				$w = $_POST["Filtre"];
				$Filtre .= " AND (1=2 or sys_classe_in_code.class_nom like '%$w%') ";
			}
			...
			$FROM .= "  $Filtre ";
			$order = " order by sys_classe_in_code.id DESC";
			...
			$taille = $_SESSION["Classe_in_Code"]["t"] = (isset($_POST["taille_pg"])and $_POST["taille_pg"]!="")?$_POST["taille_pg"]:(isset($_SESSION["Classe_in_Code"]["t"])?$_SESSION["Classe_in_Code"]["t"]:10);
			...
			if($pdo_result = self::$conx->query($Select.$FROM.$order.self::$limit)){
				...
				<table style='margin-top:4px'>
					<caption style='".self::$style_caption."'><b>Liste de Classe_in_Codes</b></caption><tr><th></th><th>N°</th><th>Classe In Code</th>".
				...
				while($row		= $pdo_result->fetch(PDO::FETCH_ASSOC)){
					...
					echo "<td>".($i++)."</td><td>$row[class_nom]</td>";
					eval($crud);
					echo "</tr>";
				}
				echo '</table>';			
				paginner(self::$count,$active_pg,$taille,'Classe_in_Code',self::$id_div_liste,false);
				echo'</div>';
			}
			else if(self::$conx->errorInfo()[2]!="") {echo self::$notification .= self::$conx->errorInfo()[2];}
		}
		
		public static function insert($class_nom){
			$id_visite = isset($_SESSION['sys_visites']['id'])?$_SESSION['sys_visites']['id']:0;
			$req = "INSERT INTO ".self::$table."(class_nom,id_visite) VALUES(" .self::$conx->quote($class_nom). ",'$id_visite');";
		...
		function creer() {
			if( true  and ''!=$_POST["class_nom"]){
				if(self::insert($_POST["class_nom"]))
					self::$notification .= self::$notifInsSucc." <form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' ><input type='submit'  name='bt_detail_".self::$table."' value='Détailler dernier(ère) Classe_in_Code' class='".self::$bt_class."' /><input type='hidden' name='id' value='".self::$id."' />".self::$marqueur." </form> "; 
		...
		function modifier(){
			if(isset($_POST['id'])and (($id = $_POST['id'])!='')){
				if( true  and ''!=$_POST["class_nom"]){
					$req = ("UPDATE ".self::$table." SET class_nom = ".self::$conx->quote($_POST["class_nom"])."  WHERE id = $id;");
		...
		//insertion dans la base de données
		function traitement() {
			if(isset($_POST['class'])and $_POST['class']!='Classe_in_Code')return false;
		...
		function form($opt,$array){
				...
				$cle='class_nom';$class_nom=isset($array["$cle"])?$array["$cle"]:''; 
			...
			if($opt=="sup"  ){
				if(isset($_POST['id'])){
					...
					"<form id='' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='border:solid 0px;'>".
						"<table><caption style='".self::$style_caption."'><b>Suppression Classe_in_Code</caption><tr><td >Voulez-vous réellement supprimer ? $ch_input<tr><td style='text-align:center'>
				...
				"<form enctype='multipart/form-data' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style=''>".
					"$ch_input".
					"<table>
						<caption style='".self::$style_caption."'><b>".($opt=='ajt'?'Enregistrement':'Modification')." Classe_in_Code</caption>".
					"<tr>".
						"<td><label for='class_nom' >Classe In Code</label></td><td>:</td>".
						"<td><input type='text' value=\"$class_nom\" title=\"Classe In Code\" placeholder=\"Classe In Code\" id='class_nom' name='class_nom' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>".  
		...
		public static function options($where,$id) {
			$req = "SELECT sys_classe_in_code.id Value ,concat(sys_classe_in_code.class_nom) DisplayText FROM ".self::$table ." sys_classe_in_code  $where ";
			if($pdo_result = self::$conx->query($req)){
				$opt  ='';
				while($row = $pdo_result->fetch(PDO::FETCH_ASSOC)){
		...
		public static function ini_champ(){
			$T_CH = self::$T_CH = array("class_nom");
			$l=0;$ch_alias="c";$label="l";$type="t";
			self::$TLBL = array(
			$T_CH[$l++]=>array($ch_alias=>"sys_classe_in_code.class_nom class_nom",$label=>"Classe In Code",$type=>"text"));
		...
		public static function bt_rapport($class,$val){
			if(!self::$readOnly){echo "
				<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
					<input type='submit' class='$class' id='bt_rapport_complet_".self::$table."' name='bt_rapport_complet_".self::$table."' value='".($val!=''?$val:'Rapport Classe_in_Code')."' onClick=\"".self::codeOnCallRapport()."\"/>
					".self::$marqueur."
				</form>";
			}
		}
		public static function rapport() {
			...
			$req = "SELECT sys_classe_in_code.id $CONSTR[Ch_Select] FROM ".self::$table ." sys_classe_in_code  ";
		...
		public static function form_rech_rapport(){
			...
			$T_SGO = self::$T_SGO;
			echo"
			<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."'>".(!(isset(self::$CHAMPS['class_nom'][$T_SGO[3]])and self::$CHAMPS['class_nom'][$T_SGO[3]])?"":"<input type='text' name='sys_classe_in_code_class_nom' title=\"Classe In Code\" placeholder=\"Classe In Code\" class='".self::$input_class."' />")
		...
		public static function bt_rapport_config($class,$val){
			echo "
			<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
				<input type='submit' class='$class' id='bt_rapport_config_".self::$table."' name='bt_rapport_config_".self::$table."' value='".($val!=''?$val:'Rapport Config Classe_in_Code')."' onClick=\"".self::codeOnCallConfig()."\"/>
				".self::$marqueur."
			</form>";
		}		
