<?php
	...
	class Releve extends Chado
	{
		public static $id;
		public static $bt_class = 'btn ';
		public static $input_style = 'width:250px';
		public static $input_class = 'form-control ';
		public static $cham_img= array('img_releve');
		public static $table 	= 't_releve';
		public static $marqueur = "<input type='hidden' name='class' value='Releve' >";
		public static $liste_modifiable= true;
		public static $useDetailStandard= true;
		public static $id_div_liste = 'id_div_liste_Releve';
		public static $id_div_crud = 'id_div_crud_Releve';
		public static $retourHtmlRapConfig = 'div_form_rapp_Releve';
		...
		public static function liste_objet($where,$limit){
			$ch_id = "rlv.id";
			$Select = "SELECT $ch_id, bq.nom_banque ,rlv.date_paiement,us.nom ,rlv.valide,rlv.img_releve,rlv.en_cours_dedition,rlv.nbr_note FROM ".self::$table ." rlv ";
			$req = "$Select 
				INNER JOIN t_banque bq ON  rlv.id_banque = bq.id 
				INNER JOIN t_user us ON  rlv.id_user = us.id  $where $limit";
			
			$count = "SELECT count(*) nbre FROM ".self::$table ." rlv ";
			$req_count = "$count 
				INNER JOIN t_banque bq ON  rlv.id_banque = bq.id 
				INNER JOIN t_user us ON  rlv.id_user = us.id  $where ";
			...
			if($pdo_result = self::$conx->query($req)){
				...
				while($row	= $pdo_result->fetch(PDO::FETCH_ASSOC)){
					...
					$row['src_img_releve'] = self::$upload."/Releve/img_releve/$row[img_releve]";
					$row['img_releve'] = "<img src='".self::$upload."/Releve/img_releve/$row[img_releve]' style='width:100px;display:block' />";
					$row['check_en_cours_dedition'] = "<input type='checkbox' readonly ".($row["en_cours_dedition"]?'checked':'')." title=\"En cour d'édition?\" />"; 
					$rows[] = $row;
				}
				return $rows;
			}
		...
		function detail($id){
			...
			if($pdo_result = self::$conx->query($req)){
				...
				<table><caption style='".self::$style_caption."'><b>Détails sur Releve</b></caption>
					...
					<tr><th>Image</th><td>:</td><td><img src='".self::$upload."/Releve/img_releve/$row[img_releve]' style='width:100px;display:block' /></td></tr>
					<tr><th>En cour d'édition?</th><td>:</td><td><input type='checkbox' readonly ".($row["en_cours_dedition"]?'checked':'')." title=\"En cour d'édition?\" /></td></tr>
					<tr><th>Nbre de note</th><td>:</td><td>$row[nbr_note]</td></tr>
					</tbody>
				</table>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Releve ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
				...
				echo " <label class='".self::$bt_class."'  id='bt_termier_".self::$table."'  onClick=\"cache1_et_affiche2('".self::$id_div_crud."','".self::$retourHtmlAjaxLIST."');\" style='display:inline-block' >Retourner à la liste de Releve</label>
				</div></div>";
		...
		function detail_personnaliser($id){
			...
			if($pdo_result = self::$conx->query($req)){
				$row		= $pdo_result->fetch(PDO::FETCH_ASSOC);
				echo"<div id='".self::$id_div_crud."' style='' >
				<p><caption style='".self::$style_caption."'><b>Détails sur Releve</b></caption>Banque : $row[nom_banque] Payé le : $row[date_paiement] Utilisateur : $row[nom] Valide ? : <input type='checkbox' readonly ".($row["valide"]?'checked':'')." title=\"Valide ?\" /> Image : <img src='".self::$upload."/Releve/img_releve/$row[img_releve]' style='width:100px;display:block' />En cour d'édition? : <input type='checkbox' readonly ".($row["en_cours_dedition"]?'checked':'')." title=\"En cour d'édition?\" /> Nbre de note : $row[nbr_note] 
				</p>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Releve ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
		...
		function form_rech_avancee(){
			...
			<form id='form_rechAc_".self::$table."' method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:none' > 
				...
				<input style='width:100px' type='text' name='rlv_date_paiement' title=\"  AAAA-MM-JJ:AAAA-MM-JJ Payé le\" placeholder=\"Payé le\"  class='".self::$input_class."'/>
				<input style='width:100px' type='text' name='us_nom' title=\"Utilisateur\" placeholder=\"Utilisateur\" class='".self::$input_class."'/>
				<input style='width:100px' type='text' name='rlv_nbr_note'  title=\"Nbre de note\" placeholder=\"Nbre de note\" class='".self::$input_class."'/>
		...
		function table_liste(){
			$Select = "SELECT rlv.id ,rlv.is_deleted ,bq.nom_banque ,rlv.date_paiement,us.nom ,rlv.valide,rlv.en_cours_dedition,rlv.nbr_note ";
			$FROM = " FROM ".self::$table ." rlv ";
			$Filtre = " WHERE 1=1 ";
			if(isset($_POST["Filtre"]) and "" != $_POST["Filtre"]){
				$w = $_POST["Filtre"];
				$Filtre .= " AND (1=2 or bq.nom_banque like '%$w%' or  rlv.date_paiement like '%$w%' or  us.nom like '%$w%' or  rlv.valide like '%$w%' or  rlv.en_cours_dedition like '%$w%' or  rlv.nbr_note like '%$w%') ";
			}
			if(isset($_POST["bt_lancer_actAv_".self::$table])){
			$Filtre .= " AND (1=1 " 
			.(trim($t = $_POST["bq_nom_banque"])!=''?(" AND bq.nom_banque LIKE ". self::$conx->quote("%$t%")):"") 
			.(((($count=(count($t=explode(':',($trim = trim($_POST["rlv_date_paiement"]))))))==1and $trim!='') or $count==2)?(" AND rlv.date_paiement BETWEEN ".self::$conx->quote($t[0])." AND ".self::$conx->quote($t[count($t)-1])):"" )
			.(trim($t = $_POST["us_nom"])!=''?(" AND us.nom LIKE ". self::$conx->quote("%$t%")):"") 
			.(trim($t=$_POST["rlv_nbr_note"])!=''?(" AND rlv.nbr_note LIKE ".self::$conx->quote("%$t%")):"" )
			.")";
			}
			$rows= array(); $recordCount= 0;
			$FROM .= " 
				INNER JOIN t_banque bq ON  rlv.id_banque = bq.id 
				INNER JOIN t_user us ON  rlv.id_user = us.id  $Filtre ";
			$order = " order by en_cours_dedition desc,rlv.id DESC";
			...
			$taille = $_SESSION["Releve"]["t"] = (isset($_POST["taille_pg"])and $_POST["taille_pg"]!="")?$_POST["taille_pg"]:(isset($_SESSION["Releve"]["t"])?$_SESSION["Releve"]["t"]:10);
			...
			if($pdo_result = self::$conx->query($Select.$FROM.$order.self::$limit)){
				...
				<table style='margin-top:4px'>
					<caption style='".self::$style_caption."'><b>Liste de Releves</b></caption><tr><th></th><th>N°</th><th>Banque</th><th>Payé le</th><th>Utilisateur</th><th>Valide ?</th><th>En cour d'édition?</th><th>Nbre de note</th>".
				...
				while($row		= $pdo_result->fetch(PDO::FETCH_ASSOC)){
					...
					echo "<td>".($i++)."</td><td>$row[nom_banque]</td><td>$row[date_paiement]</td><td>$row[nom]</td><td><input type='checkbox' readonly ".($row["valide"]?'checked':'')." title=\"Valide ?\" /></td><td><input type='checkbox' readonly ".($row["en_cours_dedition"]?'checked':'')." title=\"En cour d'édition?\" /></td><td>$row[nbr_note]</td>";
					eval($crud);
					echo "</tr>";
				}
				echo '</table>';			
				paginner(self::$count,$active_pg,$taille,'Releve',self::$id_div_liste,false);
				echo'</div>';
			}
			else if(self::$conx->errorInfo()[2]!="") {echo self::$notification .= self::$conx->errorInfo()[2];}
		}
		
		public static function insert($id_banque,$date_paiement,$id_user,$valide,$en_cours_dedition,$nbr_note){
			$id_visite = isset($_SESSION['sys_visites']['id'])?$_SESSION['sys_visites']['id']:0;
			$req = "INSERT INTO ".self::$table."(id_banque,date_paiement,id_user,valide,en_cours_dedition,nbr_note,id_visite) VALUES(" .self::$conx->quote($id_banque).','.self::$conx->quote($date_paiement).','.self::$conx->quote($id_user).','.self::$conx->quote($valide).','.self::$conx->quote($en_cours_dedition).','.self::$conx->quote($nbr_note). ",'$id_visite');";
			if(self::$conx->exec("update ".self::$table." set en_cours_dedition=0; ".$req)){
				$_SESSION[self::$table]['id'] = self::$id = self::$conx->lastInsertId();
				// self::$notification .= $req;
				return true;
		...
		function creer() {
			if( true  and (''!=($_POST["id_banque"]=($_POST["id_banque"]==''?(isset($_SESSION['t_banque']['id'])?$_SESSION['t_banque']['id']:''):$_POST["id_banque"]))) and ''!=$_POST["date_paiement"] and ''!=$_POST["nbr_note"]){
				if(self::insert($_POST["id_banque"],$_POST["date_paiement"],$_SESSION['snp']['user']['id'],1,1,$_POST["nbr_note"]))
					self::$notification .= self::$notifInsSucc." <form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' ><input type='submit'  name='bt_detail_".self::$table."' value='Détailler dernier(ère) Releve' class='".self::$bt_class."' /><input type='hidden' name='id' value='".self::$id."' />".self::$marqueur." </form> "; 
		...
		function modifier(){
			if(isset($_POST['id'])and (($id = $_POST['id'])!='')){
				if( true  and (''!=($_POST["id_banque"]=($_POST["id_banque"]==''?(isset($_SESSION['t_banque']['id'])?$_SESSION['t_banque']['id']:''):$_POST["id_banque"]))) and ''!=$_POST["date_paiement"] and ''!=$_POST["nbr_note"]){
					$req = ("UPDATE ".self::$table." SET id_banque = ".self::$conx->quote($_POST["id_banque"])." ,date_paiement = ".self::$conx->quote($_POST["date_paiement"])." ,valide = '".isset($_POST["valide"])."' ,en_cours_dedition = '".isset($_POST["en_cours_dedition"])."' ,nbr_note = ".self::$conx->quote($_POST["nbr_note"])."  WHERE id = $id;");
					$req1 = isset($_POST["en_cours_dedition"])?"update ".self::$table." set en_cours_dedition=0;":"";
					if(self::$conx->exec($req1.$req) or self::$conx->errorInfo()[2]==''){
		...
		//insertion dans la base de données
		function traitement() {
			if(isset($_POST['class'])and $_POST['class']!='Releve')return false;
			...
			if(isset($_POST['bt_sup_'.self::$table]))$this->supprimer();
				...
				if($opt=='ajt'or $opt=='mod')
					echo"<script>". self::autoCompleter('id_banque').self::autoCompleter('id_user'). "</script>";
		...
		function form($opt,$array){
				...
				$cle='id_banque';$id_banque=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='date_paiement';$date_paiement=isset($array["$cle"])?$array["$cle"]:date('Y-m-d'); 
				$cle='valide';$valide=isset($array["$cle"])?$array["$cle"]:1; 
				$cle='img_releve';$img_releve=isset($array["$cle"])?$array["$cle"]:1; 
				$cle='en_cours_dedition';$en_cours_dedition=isset($array["$cle"])?$array["$cle"]:1; 
				$cle='nbr_note';$nbr_note=isset($array["$cle"])?$array["$cle"]:2; 
			...
			if($opt=="sup"  ){
				if(isset($_POST['id'])){
					...
					"<form id='' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='border:solid 0px;'>".
						"<table><caption style='".self::$style_caption."'><b>Suppression Releve</caption><tr><td >Voulez-vous réellement supprimer ? $ch_input<tr><td style='text-align:center'>
				...
				$form = "<div id='".Banque::$id_div_crud."' ></div><br/>".Banque::bt_ajout(Banque::$bt_class,'Ajouter Banque',false)."<div id='".User::$id_div_crud."' ></div><br/>".User::bt_ajout(User::$bt_class,'Ajouter User',false).
				"<form enctype='multipart/form-data' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style=''>".
					"$ch_input".
					"<table>
						<caption style='".self::$style_caption."'><b>".($opt=='ajt'?'Enregistrement':'Modification')." Releve</caption>".
					"<tr>".
						"<td><label for='id_banque' >Banque</label></td><td>:</td>".
						"<td><select id='id_banque' name='id_banque'  class='".self::$input_class."' style='".self::$input_style."' ><option value=''>Choisir</option>" .Banque::options('',$id_banque)."</select></td>".
					"</tr>". 
					"<tr>".
						"<td><label for='date_paiement' >Payé le</label></td><td>:</td>".
						"<td><input type='date' value=\"$date_paiement\" title=\"Payé le\" placeholder=\"Payé le\" id='date_paiement' name='date_paiement' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>". 
					($opt=='mod'?("<tr>".
					...
					"</tr>"):''). 
					"<tr>".
						"<td><label for='img_releve' >Image</label></td><td>:</td>".
						"<td>".($img_releve!=''?
							"<img src='".self::$upload."/Releve/img_releve/$img_releve' style='width:100px;display:block' />":'')."<input type='file' value=\"$img_releve\" title=\"Image\" id='img_releve' name='img_releve' class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>". 
					($opt=='mod'?("<tr>".
						"<td><label for='en_cours_dedition' >En cour d'édition?</label></td><td>:</td>".
						"<td><input type='checkbox' ".($en_cours_dedition?'checked':'')." title=\"En cour d'édition?\" id='en_cours_dedition' name='en_cours_dedition' /></td>".
					"</tr>"):''). 
					"<tr>".
						"<td><label for='nbr_note' >Nbre de note</label></td><td>:</td>".
						"<td><input type='text' value=\"$nbr_note\" title=\"Nbre de note\" placeholder=\"Nbre de note\" id='nbr_note' name='nbr_note' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>".  
		...
		public function sauver_img($champ,$id){
			$I = new Image($champ);
			if($I->sauver(self::$upload."/Releve/$champ",$id)){
		...
		public static function options($where,$id) {
			...
			$req = "SELECT rlv.id Value ,concat(bq.nom_banque ,' ',rlv.date_paiement) DisplayText FROM ".self::$table ." rlv 
				INNER JOIN t_banque bq ON  rlv.id_banque = bq.id  WHERE rlv.is_deleted=0 $where order by bq.nom_banque,rlv.date_paiement DESC";
			if($pdo_result = self::$conx->query($req)){
				$opt  ='';
				while($row = $pdo_result->fetch(PDO::FETCH_ASSOC)){
		...
		public static function ini_champ(){
			$T_CH = self::$T_CH = array("id_banque","nom_banque","date_paiement","id_user","nom","valide","img_releve","en_cours_dedition","nbr_note");
			$l=0;$ch_alias="c";$label="l";$type="t";
			self::$TLBL = array(
			$T_CH[$l++]=>array($ch_alias=>"rlv.id_banque id_banque",$label=>"Banque",$type=>"select"),
			$T_CH[$l++]=>array($ch_alias=>"bq.nom_banque nom_banque",$label=>"Banque",$type=>""),
			$T_CH[$l++]=>array($ch_alias=>"rlv.date_paiement date_paiement",$label=>"Payé le",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"rlv.id_user id_user",$label=>"Utilisateur",$type=>"select"),
			$T_CH[$l++]=>array($ch_alias=>"us.nom nom",$label=>"Utilisateur",$type=>""),
			$T_CH[$l++]=>array($ch_alias=>"rlv.valide valide",$label=>"Valide ?",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"rlv.img_releve img_releve",$label=>"Image",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"rlv.en_cours_dedition en_cours_dedition",$label=>"En cour d'édition?",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"rlv.nbr_note nbr_note",$label=>"Nbre de note",$type=>"text"));
		...
		public static function bt_rapport($class,$val){
			if(!self::$readOnly){echo "
				<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
					<input type='submit' class='$class' id='bt_rapport_complet_".self::$table."' name='bt_rapport_complet_".self::$table."' value='".($val!=''?$val:'Rapport Releve')."' onClick=\"".self::codeOnCallRapport()."\"/>
					".self::$marqueur."
				</form>";
			}
		}
		public static function rapport() {
			...
			$req = "SELECT rlv.id $CONSTR[Ch_Select] FROM ".self::$table ." rlv 
				INNER JOIN t_banque bq ON  rlv.id_banque = bq.id 
				INNER JOIN t_user us ON  rlv.id_user = us.id  ";
			self::suite_rapport($req,$CONSTR,self::$table,self::$marqueur,self::$style_caption,7);
		}
		...
		public static function form_rech_rapport(){
			...
			$T_SGO = self::$T_SGO;
			echo"
			<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."'>".(!(isset(self::$CHAMPS['id_banque'][$T_SGO[3]])and self::$CHAMPS['id_banque'][$T_SGO[3]])?"":"<select title=\"Banque\" id='rlv_id_banque' name='rlv_id_banque'  class='".self::$input_class."'><option value=''>Choisir</option>".Banque::options('',0)."</select>").
				...
				(!(isset(self::$CHAMPS['date_paiement'][$T_SGO[3]])and self::$CHAMPS['date_paiement'][$T_SGO[3]])?"":"<input type='text' name='rlv_date_paiement' title=\"Payé le\" placeholder=\"Payé le\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['id_user'][$T_SGO[3]])and self::$CHAMPS['id_user'][$T_SGO[3]])?"":"<select title=\"Utilisateur\" id='rlv_id_user' name='rlv_id_user'  class='".self::$input_class."'><option value=''>Choisir</option>".User::options('',0)."</select>").
				...
				(!(isset(self::$CHAMPS['valide'][$T_SGO[3]])and self::$CHAMPS['valide'][$T_SGO[3]])?"":"<input type='text' name='rlv_valide' title=\"Valide ?\" placeholder=\"Valide ?\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['img_releve'][$T_SGO[3]])and self::$CHAMPS['img_releve'][$T_SGO[3]])?"":"<input type='text' name='rlv_img_releve' title=\"Image\" placeholder=\"Image\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['en_cours_dedition'][$T_SGO[3]])and self::$CHAMPS['en_cours_dedition'][$T_SGO[3]])?"":"<input type='text' name='rlv_en_cours_dedition' title=\"En cour d'édition?\" placeholder=\"En cour d'édition?\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['nbr_note'][$T_SGO[3]])and self::$CHAMPS['nbr_note'][$T_SGO[3]])?"":"<input type='text' name='rlv_nbr_note' title=\"Nbre de note\" placeholder=\"Nbre de note\" class='".self::$input_class."' />")
		...
		public static function bt_rapport_config($class,$val){
			echo "
			<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
				<input type='submit' class='$class' id='bt_rapport_config_".self::$table."' name='bt_rapport_config_".self::$table."' value='".($val!=''?$val:'Rapport Config Releve')."' onClick=\"".self::codeOnCallConfig()."\"/>
				".self::$marqueur."
			</form>";
		}		
