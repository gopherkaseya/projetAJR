<?php
	...
	class Service extends Chado
	{
		...
		public static $table 	= 't_service';
		public static $marqueur = "<input type='hidden' name='class' value='Service' >";
		public static $liste_modifiable= true;
		public static $useDetailStandard= true;
		public static $id_div_liste = 'id_div_liste_Service';
		public static $id_div_crud = 'id_div_crud_Service';
		public static $retourHtmlRapConfig = 'div_form_rapp_Service';
		...
		public static function liste_objet($where,$limit){
			$ch_id = "ser.id";
			$Select = "SELECT $ch_id, ser.service,ser.s_car_nbr_limit FROM ".self::$table ." ser ";
			$req = "$Select  $where $limit";
			
			$count = "SELECT count(*) nbre FROM ".self::$table ." ser ";
		...
		function detail($id){
			...
			if($pdo_result = self::$conx->query($req)){
				...
				<table><caption style='".self::$style_caption."'><b>Détails sur Service</b></caption>
					<tbody style='text-align:left'>
					<tr><th>Service</th><td>:</td><td>$row[service]</td></tr>
					<tr><th>Nbr.Limt de carnet encours</th><td>:</td><td>$row[s_car_nbr_limit]</td></tr>
					</tbody>
				</table>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Service ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
				...
				echo " <label class='".self::$bt_class."'  id='bt_termier_".self::$table."'  onClick=\"cache1_et_affiche2('".self::$id_div_crud."','".self::$retourHtmlAjaxLIST."');\" style='display:inline-block' >Retourner à la liste de Service</label>
				</div></div>";
		...
		function detail_personnaliser($id){
			...
			if($pdo_result = self::$conx->query($req)){
				$row		= $pdo_result->fetch(PDO::FETCH_ASSOC);
				echo"<div id='".self::$id_div_crud."' style='' >
				<p><caption style='".self::$style_caption."'><b>Détails sur Service</b></caption>Service : $row[service] Nbr.Limt de carnet encours : $row[s_car_nbr_limit] 
				</p>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Service ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
		...
		function form_rech_avancee(){
			...
			<form id='form_rechAc_".self::$table."' method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:none' > 
				...
				<input style='width:100px' type='text' name='ser_s_car_nbr_limit'  title=\"Nbr.Limt de carnet encours\" placeholder=\"Nbr.Limt de carnet encours\" class='".self::$input_class."'/>
		...
		function table_liste(){
			$Select = "SELECT ser.id ,ser.is_deleted ,ser.service,ser.sous_service,ser.s_car_nbr_limit ";
			$FROM = " FROM ".self::$table ." ser ";
			$Filtre = " WHERE 1=1 ";
			if(isset($_POST["Filtre"]) and "" != $_POST["Filtre"]){
				$w = $_POST["Filtre"];
				$Filtre .= " AND (1=2 or ser.service like '%$w%' or  ser.s_car_nbr_limit like '%$w%') ";
			}
			if(isset($_POST["bt_lancer_actAv_".self::$table])){
			$Filtre .= " AND (1=1 " 
			.(trim($t=$_POST["ser_service"])!=''?(" AND ser.service LIKE ".self::$conx->quote("%$t%")):"" )
			.(trim($t=$_POST["ser_s_car_nbr_limit"])!=''?(" AND ser.s_car_nbr_limit LIKE ".self::$conx->quote("%$t%")):"" )
			.")";
			}
			$rows= array(); $recordCount= 0;
			$FROM .= "  $Filtre ";
			$order = " order by ser.id DESC";
			...
			$taille = $_SESSION["Service"]["t"] = (isset($_POST["taille_pg"])and $_POST["taille_pg"]!="")?$_POST["taille_pg"]:(isset($_SESSION["Service"]["t"])?$_SESSION["Service"]["t"]:10);
			...
			if($pdo_result = self::$conx->query($Select.$FROM.$order.self::$limit)){
				...
				<table style='margin-top:4px'>
					<caption style='".self::$style_caption."'><b>Liste de Services</b></caption><tr><th></th><th>N°</th><th>Service</th><th>Sous-Service</th><th>Nbr.Limt de carnet encours</th>".
				...
				while($row		= $pdo_result->fetch(PDO::FETCH_ASSOC)){
					...
					echo "<td>".($i++)."</td><td>$row[service]</td><td>$row[sous_service]</td><td>$row[s_car_nbr_limit]</td>";
					eval($crud);
					echo "</tr>";
				}
				echo '</table>';			
				paginner(self::$count,$active_pg,$taille,'Service',self::$id_div_liste,false);
				echo'</div>';
			}
			else if(self::$conx->errorInfo()[2]!="") {echo self::$notification .= self::$conx->errorInfo()[2];}
		}
		
		public static function insert($service,$sous_service,$s_car_nbr_limit){
			$id_visite = isset($_SESSION['sys_visites']['id'])?$_SESSION['sys_visites']['id']:0;
			$req = "INSERT INTO ".self::$table."(service,sous_service,s_car_nbr_limit,id_visite) VALUES(" .self::$conx->quote($service).','.self::$conx->quote($sous_service).','.self::$conx->quote($s_car_nbr_limit). ",'$id_visite');";
		...
		function creer() {
			if( true  and ''!=$_POST["service"] and ''!=$_POST["s_car_nbr_limit"]){
				if(self::insert($_POST["service"],$_POST["sous_service"],$_POST["s_car_nbr_limit"]))
					self::$notification .= self::$notifInsSucc." <form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' ><input type='submit'  name='bt_detail_".self::$table."' value='Détailler dernier(ère) Service' class='".self::$bt_class."' /><input type='hidden' name='id' value='".self::$id."' />".self::$marqueur." </form> "; 
		...
		function modifier(){
			if(isset($_POST['id'])and (($id = $_POST['id'])!='')){
				if( true  and ''!=$_POST["service"] and ''!=$_POST["s_car_nbr_limit"]){
					$req = ("UPDATE ".self::$table." SET service = ".self::$conx->quote($_POST["service"])." ,s_car_nbr_limit = ".self::$conx->quote($_POST["s_car_nbr_limit"])."  WHERE id = $id;");
		...
		//insertion dans la base de données
		function traitement() {
			if(isset($_POST['class'])and $_POST['class']!='Service')return false;
		...
		function form($opt,$array){
				...
				$cle='service';$service=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='sous_service';$sous_service=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='s_car_nbr_limit';$s_car_nbr_limit=isset($array["$cle"])?$array["$cle"]:''; 
			...
			if($opt=="sup"  ){
				if(isset($_POST['id'])){
					...
					"<form id='' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='border:solid 0px;'>".
						"<table><caption style='".self::$style_caption."'><b>Suppression Service</caption><tr><td >Voulez-vous réellement supprimer ? $ch_input<tr><td style='text-align:center'>
				...
				"<form enctype='multipart/form-data' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style=''>".
					"$ch_input".
					"<table>
						<caption style='".self::$style_caption."'><b>".($opt=='ajt'?'Enregistrement':'Modification')." Service</caption>".
					"<tr>".
						"<td><label for='service' >Service</label></td><td>:</td>".
						"<td><input type='text' value=\"$service\" title=\"Service\" placeholder=\"Service\" id='service' name='service' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>". 
					"<tr>".
						"<td><label for='sous_service' >Service</label></td><td>:</td>".
						"<td><input type='text' value=\"$sous_service\" title=\"Sous-Service\" placeholder=\"Sous-Service\" id='service' name='sous_service'  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>". 
					"<tr>".
						"<td><label for='s_car_nbr_limit' >Nbr.Limt de carnet encours</label></td><td>:</td>".
						"<td><input type='text' value=\"$s_car_nbr_limit\" title=\"Nbr.Limt de carnet encours\" placeholder=\"Nbr.Limt de carnet encours\" id='s_car_nbr_limit' name='s_car_nbr_limit' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>".  
		...
		public static function options($where,$id) {
			...
			$req = "SELECT ser.id Value ,concat(ser.service) DisplayText FROM ".self::$table ." ser  WHERE ser.is_deleted=0 $where order by ser.service ";
			if($pdo_result = self::$conx->query($req)){
				$opt  ='';
				while($row = $pdo_result->fetch(PDO::FETCH_ASSOC)){
		...
		public static function ini_champ(){
			$T_CH = self::$T_CH = array("service","s_car_nbr_limit");
			$l=0;$ch_alias="c";$label="l";$type="t";
			self::$TLBL = array(
			$T_CH[$l++]=>array($ch_alias=>"ser.service service",$label=>"Service",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"ser.s_car_nbr_limit s_car_nbr_limit",$label=>"Nbr.Limt de carnet encours",$type=>"text"));
		...
		public static function bt_rapport($class,$val){
			if(!self::$readOnly){echo "
				<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
					<input type='submit' class='$class' id='bt_rapport_complet_".self::$table."' name='bt_rapport_complet_".self::$table."' value='".($val!=''?$val:'Rapport Service')."' onClick=\"".self::codeOnCallRapport()."\"/>
					".self::$marqueur."
				</form>";
			}
		}
		public static function rapport() {
			...
			$req = "SELECT ser.id $CONSTR[Ch_Select] FROM ".self::$table ." ser  ";
		...
		public static function form_rech_rapport(){
			...
			$T_SGO = self::$T_SGO;
			echo"
			<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."'>".(!(isset(self::$CHAMPS['service'][$T_SGO[3]])and self::$CHAMPS['service'][$T_SGO[3]])?"":"<input type='text' name='ser_service' title=\"Service\" placeholder=\"Service\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['s_car_nbr_limit'][$T_SGO[3]])and self::$CHAMPS['s_car_nbr_limit'][$T_SGO[3]])?"":"<input type='text' name='ser_s_car_nbr_limit' title=\"Nbr.Limt de carnet encours\" placeholder=\"Nbr.Limt de carnet encours\" class='".self::$input_class."' />")
		...
		public static function bt_rapport_config($class,$val){
			echo "
			<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
				<input type='submit' class='$class' id='bt_rapport_config_".self::$table."' name='bt_rapport_config_".self::$table."' value='".($val!=''?$val:'Rapport Config Service')."' onClick=\"".self::codeOnCallConfig()."\"/>
				".self::$marqueur."
			</form>";
		}		
