<?php
	...
	class Utilisateur extends Chado
	{
		public static $id;
		...
		public static $table 	= 'sys_utilisateur';
		public static $marqueur = "<input type='hidden' name='class' value='Utilisateur' >";
		public static $liste_modifiable= true;
		public static $useDetailStandard= true;
		public static $id_div_liste = 'id_div_liste_Utilisateur';
		public static $id_div_crud = 'id_div_crud_Utilisateur';
		public static $retourHtmlRapConfig = 'div_form_rapp_Utilisateur';
		...
		public static function liste_objet($where, $limit) {
			$ch_id = "sys_utilisateur.id";
			$Select = "SELECT $ch_id, sys_utilisateur.identite, sys_utilisateur.login, sys_utilisateur.password, sys_utilisateur.actif, sys_utilisateur.str_img FROM ".self::$table." sys_utilisateur ";
			$req = "$Select  $where $limit";
		
			// Requête pour compter les résultats
			$count = "SELECT count(*) nbre FROM ".self::$table;
			$req_count = "$count  $where";
		
			// Gestion de la requête de comptage
			try {
				$pdo_result = self::$conx->query($req_count);
				if ($pdo_result) {
					$row = $pdo_result->fetch(PDO::FETCH_ASSOC);
					self::$count = $row['nbre'];
				}
			} catch (PDOException $e) {
				// Gestion de l'erreur, enregistre l'erreur dans la notification et retourne un message d'erreur
				self::$notification .= "Erreur lors du comptage des utilisateurs : " . $e->getMessage();
				return false; // Retourne false pour indiquer une erreur
			}
		
			// Exécution de la requête principale pour récupérer les utilisateurs
			try {
				$pdo_result = self::$conx->query($req);
				if ($pdo_result) {
					$rows = array();
					while ($row = $pdo_result->fetch(PDO::FETCH_ASSOC)) {
						// Ajout des informations supplémentaires aux résultats
						$row['check_actif'] = "<input type='checkbox' readonly ".($row["actif"] ? 'checked' : '')." title=\"Actif ?\" />"; 
						$row['src_str_img'] = self::$upload."/Utilisateur/str_img/$row[str_img]";
						$row['str_img'] = "<img src='".self::$upload."/Utilisateur/str_img/$row[str_img]' style='width:100px;display:block' />";
						$rows[] = $row;
					}
					return $rows;
				} else {
					// Si la requête échoue, enregistre l'erreur et retourne un message d'erreur
					self::$notification .= "Erreur lors de l'exécution de la requête : " . self::$conx->errorInfo()[2];
					return false; // Retourne false pour indiquer une erreur
				}
			} catch (PDOException $e) {
				// Gestion de l'erreur, enregistre l'erreur dans la notification et retourne un message d'erreur
				self::$notification .= "Erreur lors de la récupération des utilisateurs : " . $e->getMessage();
				return false; // Retourne false pour indiquer une erreur
			}
		}
		...
		function detail($id){
			...
			if($pdo_result = self::$conx->query($req)){
				...
				<table><caption style='".self::$style_caption."'><b>Détails sur Utilisateur</b></caption>
					...
					<tr><th>Actif ?</th><td>:</td><td><input type='checkbox' readonly ".($row["actif"]?'checked':'')." title=\"Actif ?\" /></td></tr>
					<tr><th>Image</th><td>:</td><td><img src='".self::$upload."/Utilisateur/str_img/$row[str_img]' style='width:100px;display:block' /></td></tr>
					</tbody>
				</table>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Utilisateur ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
				...
				echo " <label class='".self::$bt_class."'  id='bt_termier_".self::$table."'  onClick=\"cache1_et_affiche2('".self::$id_div_crud."','".self::$retourHtmlAjaxLIST."');\" style='display:inline-block' >Retourner à la liste de Utilisateur</label>
				</div></div>";
		...
		function detail_personnaliser($id){
			...
			if($pdo_result = self::$conx->query($req)){
				$row		= $pdo_result->fetch(PDO::FETCH_ASSOC);
				echo"<div id='".self::$id_div_crud."' style='' >
				<p><caption style='".self::$style_caption."'><b>Détails sur Utilisateur</b></caption>Utilisateur : $row[identite] Login : $row[login] Mot De passe : $row[password] Actif ? : <input type='checkbox' readonly ".($row["actif"]?'checked':'')." title=\"Actif ?\" /> Image : <img src='".self::$upload."/Utilisateur/str_img/$row[str_img]' style='width:100px;display:block' />
				</p>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Utilisateur ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
		...
		function form_rech_avancee(){
			...
			<form id='form_rechAc_".self::$table."' method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:none' > 
				...
				<input style='width:100px' type='text' name='sys_utilisateur_login'  title=\"Login\" placeholder=\"Login\" class='".self::$input_class."'/>
				<input style='width:100px' type='text' name='sys_utilisateur_password'  title=\"Mot De passe\" placeholder=\"Mot De passe\" class='".self::$input_class."'/>
		...
		function table_liste(){
			$Select = "SELECT sys_utilisateur.id ,sys_utilisateur.is_deleted ,sys_utilisateur.identite,sys_utilisateur.login,sys_utilisateur.password,sys_utilisateur.actif ";
			$FROM = " FROM ".self::$table ." sys_utilisateur ";
			$Filtre = " WHERE 1=1 ";
			if(isset($_POST["Filtre"]) and "" != $_POST["Filtre"]){
				$w = $_POST["Filtre"];
				$Filtre .= " AND (1=2 or sys_utilisateur.identite like '%$w%' or  sys_utilisateur.login like '%$w%' or  sys_utilisateur.password like '%$w%' or  sys_utilisateur.actif like '%$w%') ";
			}
			...
			.(trim($t=$_POST["sys_utilisateur_login"])!=''?(" AND sys_utilisateur.login LIKE ".self::$conx->quote("%$t%")):"" )
			.(trim($t=$_POST["sys_utilisateur_password"])!=''?(" AND sys_utilisateur.password LIKE ".self::$conx->quote("%$t%")):"" )
			.")";
			}
			$rows= array(); $recordCount= 0;
			$FROM .= "  $Filtre ";
			$order = " order by sys_utilisateur.id DESC";
			...
			$taille = $_SESSION["Utilisateur"]["t"] = (isset($_POST["taille_pg"])and $_POST["taille_pg"]!="")?$_POST["taille_pg"]:(isset($_SESSION["Utilisateur"]["t"])?$_SESSION["Utilisateur"]["t"]:10);
			...
			if($pdo_result = self::$conx->query($Select.$FROM.$order.self::$limit)){
				...
				<table style='margin-top:4px'>
					<caption style='".self::$style_caption."'><b>Liste de Utilisateurs</b></caption><tr><th></th><th>N°</th><th>Utilisateur</th><th>Login</th><th>Mot De passe</th><th>Actif ?</th>".
				...
				while($row		= $pdo_result->fetch(PDO::FETCH_ASSOC)){
					...
					echo "<td>".($i++)."</td><td>$row[identite]</td><td>$row[login]</td><td>$row[password]</td><td><input type='checkbox' readonly ".($row["actif"]?'checked':'')." title=\"Actif ?\" /></td>";
					eval($crud);
					echo "</tr>";
				}
				echo '</table>';			
				paginner(self::$count,$active_pg,$taille,'Utilisateur',self::$id_div_liste,false);
				echo'</div>';
			}
			else if(self::$conx->errorInfo()[2]!="") {echo self::$notification .= self::$conx->errorInfo()[2];}
		}
		
		public static function insert($identite,$login,$password,$actif){
			$id_visite = isset($_SESSION['sys_visites']['id'])?$_SESSION['sys_visites']['id']:0;
			$req = "INSERT INTO ".self::$table."(identite,login,password,actif,id_visite) VALUES(" .self::$conx->quote($identite).','.self::$conx->quote($login).','.self::$conx->quote($password).','.self::$conx->quote($actif). ",'$id_visite');";
		...
		function creer() {
			if( true  and ''!=$_POST["identite"] and ''!=$_POST["login"] and ''!=$_POST["password"]){
				if(self::insert($_POST["identite"],$_POST["login"],$_POST["password"],"".(isset($_POST["actif"])?1:0)."" ))
					self::$notification .= self::$notifInsSucc." <form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' ><input type='submit'  name='bt_detail_".self::$table."' value='Détailler dernier(ère) Utilisateur' class='".self::$bt_class."' /><input type='hidden' name='id' value='".self::$id."' />".self::$marqueur." </form> "; 
		...
		function modifier(){
			if(isset($_POST['id'])and (($id = $_POST['id'])!='')){
				if( true  and ''!=$_POST["identite"] and ''!=$_POST["login"] and ''!=$_POST["password"]){
					$req = ("UPDATE ".self::$table." SET identite = ".self::$conx->quote($_POST["identite"])." ,login = ".self::$conx->quote($_POST["login"])." ,password = ".self::$conx->quote($_POST["password"])." ,actif = '".isset($_POST["actif"])."'  WHERE id = $id;");
		...
		//insertion dans la base de données
		function traitement() {
			if(isset($_POST['class'])and $_POST['class']!='Utilisateur')return false;
		...
		function form($opt,$array){
				...
				$cle='identite';$identite=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='login';$login=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='password';$password=isset($array["$cle"])?$array["$cle"]:''; 
				$cle='actif';$actif=isset($array["$cle"])?$array["$cle"]:''; 
			...
			if($opt=="sup"  ){
				if(isset($_POST['id'])){
					...
					"<form id='' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='border:solid 0px;'>".
						"<table><caption style='".self::$style_caption."'><b>Suppression Utilisateur</caption><tr><td >Voulez-vous réellement supprimer ? $ch_input<tr><td style='text-align:center'>
				...
				"<form enctype='multipart/form-data' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style=''>".
					"$ch_input".
					"<table>
						<caption style='".self::$style_caption."'><b>".($opt=='ajt'?'Enregistrement':'Modification')." Utilisateur</caption>".
					"<tr>".
						"<td><label for='identite' >Utilisateur</label></td><td>:</td>".
						"<td><input type='text' value=\"$identite\" title=\"Utilisateur\" placeholder=\"Utilisateur\" id='identite' name='identite' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>". 
					"<tr>".
					...
					"</tr>". 
					"<tr>".
						"<td><label for='actif' >Actif ?</label></td><td>:</td>".
						"<td><input type='checkbox' ".($actif?'checked':'')." title=\"Actif ?\" id='actif' name='actif' /></td>".
					"</tr>". 
					"<tr>".
						"<td><label for='str_img' >Image</label></td><td>:</td>".
						"<td>".($str_img!=''?
							"<img src='".self::$upload."/Utilisateur/str_img/$str_img' style='width:100px;display:block' />":'')."<input type='file' value=\"$str_img\" title=\"Image\" id='str_img' name='str_img' class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>".  
		...
		public function sauver_img($champ,$id){
			$I = new Image($champ);
			if($I->sauver(self::$upload."/Utilisateur/$champ",$id)){
		...
		public static function options($where,$id) {
			$req = "SELECT sys_utilisateur.id Value ,concat(sys_utilisateur.identite) DisplayText FROM ".self::$table ." sys_utilisateur  $where ";
			if($pdo_result = self::$conx->query($req)){
				$opt  ='';
				while($row = $pdo_result->fetch(PDO::FETCH_ASSOC)){
		...
		public static function ini_champ(){
			$T_CH = self::$T_CH = array("identite","login","password","actif","str_img");
			$l=0;$ch_alias="c";$label="l";$type="t";
			self::$TLBL = array(
			$T_CH[$l++]=>array($ch_alias=>"sys_utilisateur.identite identite",$label=>"Utilisateur",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"sys_utilisateur.login login",$label=>"Login",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"sys_utilisateur.password password",$label=>"Mot De passe",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"sys_utilisateur.actif actif",$label=>"Actif ?",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"sys_utilisateur.str_img str_img",$label=>"Image",$type=>"text"));
		...
		public static function bt_rapport($class,$val){
			if(!self::$readOnly){echo "
				<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
					<input type='submit' class='$class' id='bt_rapport_complet_".self::$table."' name='bt_rapport_complet_".self::$table."' value='".($val!=''?$val:'Rapport Utilisateur')."' onClick=\"".self::codeOnCallRapport()."\"/>
					".self::$marqueur."
				</form>";
			}
		}
		public static function rapport() {
			...
			$req = "SELECT sys_utilisateur.id $CONSTR[Ch_Select] FROM ".self::$table ." sys_utilisateur  ";
			self::suite_rapport($req,$CONSTR,self::$table,self::$marqueur,self::$style_caption,4);
		}
		...
		public static function form_rech_rapport(){
			...
			$T_SGO = self::$T_SGO;
			echo"
			<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."'>".(!(isset(self::$CHAMPS['identite'][$T_SGO[3]])and self::$CHAMPS['identite'][$T_SGO[3]])?"":"<input type='text' name='sys_utilisateur_identite' title=\"Utilisateur\" placeholder=\"Utilisateur\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['login'][$T_SGO[3]])and self::$CHAMPS['login'][$T_SGO[3]])?"":"<input type='text' name='sys_utilisateur_login' title=\"Login\" placeholder=\"Login\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['password'][$T_SGO[3]])and self::$CHAMPS['password'][$T_SGO[3]])?"":"<input type='text' name='sys_utilisateur_password' title=\"Mot De passe\" placeholder=\"Mot De passe\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['actif'][$T_SGO[3]])and self::$CHAMPS['actif'][$T_SGO[3]])?"":"<input type='text' name='sys_utilisateur_actif' title=\"Actif ?\" placeholder=\"Actif ?\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['str_img'][$T_SGO[3]])and self::$CHAMPS['str_img'][$T_SGO[3]])?"":"<input type='text' name='sys_utilisateur_str_img' title=\"Image\" placeholder=\"Image\" class='".self::$input_class."' />")
		...
		public static function bt_rapport_config($class,$val){
			echo "
			<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
				<input type='submit' class='$class' id='bt_rapport_config_".self::$table."' name='bt_rapport_config_".self::$table."' value='".($val!=''?$val:'Rapport Config Utilisateur')."' onClick=\"".self::codeOnCallConfig()."\"/>
				".self::$marqueur."
			</form>";
		}		
