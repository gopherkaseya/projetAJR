<?php
	...
	class Visites extends Chado
	{
		...
		public static $table 	= 'sys_visites';
		public static $marqueur = "<input type='hidden' name='class' value='Visites' >";
		public static $liste_modifiable= true;
		public static $useDetailStandard= true;
		public static $id_div_liste = 'id_div_liste_Visites';
		public static $id_div_crud = 'id_div_crud_Visites';
		public static $retourHtmlRapConfig = 'div_form_rapp_Visites';
		...
		public static function liste_objet($where,$limit){
			$ch_id = "sys_visites.id";
			$Select = "SELECT $ch_id, sys_utilisateur.identite ,sys_visites.date_heure_visite,sys_visites.date_heure_deconx FROM ".self::$table ." sys_visites ";
			$req = "$Select 
				INNER JOIN sys_utilisateur sys_utilisateur ON  sys_visites.id_utilisateur = sys_utilisateur.id  $where $limit";
			
			$count = "SELECT count(*) nbre FROM ".self::$table ." sys_visites ";
			$req_count = "$count 
				INNER JOIN sys_utilisateur sys_utilisateur ON  sys_visites.id_utilisateur = sys_utilisateur.id  $where ";
		...
		function detail($id){
			...
			if($pdo_result = self::$conx->query($req)){
				...
				<table><caption style='".self::$style_caption."'><b>Détails sur Visites</b></caption>
					<tbody style='text-align:left'>
					<tr><th>Utilisateur</th><td>:</td><td>$row[identite]</td></tr>
					<tr><th>Connecté le</th><td>:</td><td>$row[date_heure_visite]</td></tr>
					<tr><th>déConnecté le</th><td>:</td><td>$row[date_heure_deconx]</td></tr>
					</tbody>
				</table>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Visites ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
				...
				echo " <label class='".self::$bt_class."'  id='bt_termier_".self::$table."'  onClick=\"cache1_et_affiche2('".self::$id_div_crud."','".self::$retourHtmlAjaxLIST."');\" style='display:inline-block' >Retourner à la liste de Visites</label>
				</div></div>";
		...
		function detail_personnaliser($id){
			...
			if($pdo_result = self::$conx->query($req)){
				$row		= $pdo_result->fetch(PDO::FETCH_ASSOC);
				echo"<div id='".self::$id_div_crud."' style='' >
				<p><caption style='".self::$style_caption."'><b>Détails sur Visites</b></caption>Utilisateur : $row[identite] Connecté le : $row[date_heure_visite] déConnecté le : $row[date_heure_deconx] 
				</p>
				".((!self::$readOnly)?
				"<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block'  ><input type='submit'  name='bt_lancer_mod_".self::$table."' value='Modifier Visites ?' class='".self::$bt_class." btn-primary' /><input type='hidden' name='id' value='$row[id]' />".self::$marqueur." </form>":"");
		...
		function form_rech_avancee(){
			...
			<form id='form_rechAc_".self::$table."' method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:none' > 
				...
				<input style='width:100px' type='text' name='sys_visites_date_heure_visite' title=\"  AAAA-MM-JJ:AAAA-MM-JJ Connecté le\" placeholder=\"Connecté le\"  class='".self::$input_class."'/>
				<input style='width:100px' type='text' name='sys_visites_date_heure_deconx' title=\"  AAAA-MM-JJ:AAAA-MM-JJ déConnecté le\" placeholder=\"déConnecté le\"  class='".self::$input_class."'/>
		...
		function table_liste(){
			$Select = "SELECT sys_visites.id ,sys_visites.is_deleted ,sys_utilisateur.identite ,sys_visites.date_heure_visite,sys_visites.date_heure_deconx ";
			$FROM = " FROM ".self::$table ." sys_visites ";
			$Filtre = " WHERE 1=1 ";
			if(isset($_POST["Filtre"]) and "" != $_POST["Filtre"]){
				$w = $_POST["Filtre"];
				$Filtre .= " AND (1=2 or sys_utilisateur.identite like '%$w%' or  sys_visites.date_heure_visite like '%$w%' or  sys_visites.date_heure_deconx like '%$w%') ";
			}
			...
			.(((($count=(count($t=explode(':',($trim = trim($_POST["sys_visites_date_heure_visite"]))))))==1and $trim!='') or $count==2)?(" AND sys_visites.date_heure_visite BETWEEN ".self::$conx->quote($t[0])." AND ".self::$conx->quote($t[count($t)-1])):"" )
			.(((($count=(count($t=explode(':',($trim = trim($_POST["sys_visites_date_heure_deconx"]))))))==1and $trim!='') or $count==2)?(" AND sys_visites.date_heure_deconx BETWEEN ".self::$conx->quote($t[0])." AND ".self::$conx->quote($t[count($t)-1])):"" )
			.")";
			}
			$rows= array(); $recordCount= 0;
			$FROM .= " 
				INNER JOIN sys_utilisateur sys_utilisateur ON  sys_visites.id_utilisateur = sys_utilisateur.id  $Filtre ";
			$order = " order by sys_visites.id DESC";
			...
			$taille = $_SESSION["Visites"]["t"] = (isset($_POST["taille_pg"])and $_POST["taille_pg"]!="")?$_POST["taille_pg"]:(isset($_SESSION["Visites"]["t"])?$_SESSION["Visites"]["t"]:10);
			...
			if($pdo_result = self::$conx->query($Select.$FROM.$order.self::$limit)){
				...
				<table style='margin-top:4px'>
					<caption style='".self::$style_caption."'><b>Liste de Visitess</b></caption><tr><th></th><th>N°</th><th>Utilisateur</th><th>Connecté le</th><th>déConnecté le</th>".
				...
				while($row		= $pdo_result->fetch(PDO::FETCH_ASSOC)){
					...
					echo "<td>".($i++)."</td><td>$row[identite]</td><td>$row[date_heure_visite]</td><td>$row[date_heure_deconx]</td>";
					eval($crud);
					echo "</tr>";
				}
				echo '</table>';			
				paginner(self::$count,$active_pg,$taille,'Visites',self::$id_div_liste,false);
				echo'</div>';
			}
			else if(self::$conx->errorInfo()[2]!="") {echo self::$notification .= self::$conx->errorInfo()[2];}
		}
		
		public static function insert($id_utilisateur,$date_heure_visite,$date_heure_deconx){
			$id_visite = isset($_SESSION['sys_visites']['id'])?$_SESSION['sys_visites']['id']:0;
			$req = "INSERT INTO ".self::$table."(id_utilisateur,date_heure_visite,date_heure_deconx,id_visite) VALUES(" .self::$conx->quote($id_utilisateur).','.self::$conx->quote($date_heure_visite).','.self::$conx->quote($date_heure_deconx). ",'$id_visite');";
		...
		function creer() {
			if( true ){
				if(self::insert($_SESSION['id_utilisateur'],date('Y-m-d H:i:s'),$_POST["date_heure_deconx"]))
					self::$notification .= self::$notifInsSucc." <form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."' ><input type='submit'  name='bt_detail_".self::$table."' value='Détailler dernier(ère) Visites' class='".self::$bt_class."' /><input type='hidden' name='id' value='".self::$id."' />".self::$marqueur." </form> "; 
		...
		function modifier(){
			if(isset($_POST['id'])and (($id = $_POST['id'])!='')){
				if( true  and (''!=($_POST["id_utilisateur"]=($_POST["id_utilisateur"]==''?(isset($_SESSION['sys_utilisateur']['id'])?$_SESSION['sys_utilisateur']['id']:''):$_POST["id_utilisateur"]))) and ''!=$_POST["date_heure_visite"]){
					$req = ("UPDATE ".self::$table." SET id_utilisateur = ".self::$conx->quote($_POST["id_utilisateur"])." ,date_heure_visite = ".self::$conx->quote($_POST["date_heure_visite"])." ,date_heure_deconx = ".self::$conx->quote($_POST["date_heure_deconx"])."  WHERE id = $id;");
		...
		//insertion dans la base de données
		function traitement() {
			if(isset($_POST['class'])and $_POST['class']!='Visites')return false;
			...
			if(isset($_POST['bt_sup_'.self::$table]))$this->supprimer();
				...
				if($opt=='ajt'or $opt=='mod')
					echo"<script>". self::autoCompleter('id_utilisateur'). "</script>";
		...
		function form($opt,$array){
				...
				$cle='id_utilisateur';$id_utilisateur=isset($array["$cle"])?$array["$cle"]:$_SESSION['id_utilisateur']; 
				$cle='date_heure_visite';$date_heure_visite=isset($array["$cle"])?$array["$cle"]:date('Y-m-d H:i:s'); 
				$cle='date_heure_deconx';$date_heure_deconx=isset($array["$cle"])?$array["$cle"]:''; 
			...
			if($opt=="sup"  ){
				if(isset($_POST['id'])){
					...
					"<form id='' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='border:solid 0px;'>".
						"<table><caption style='".self::$style_caption."'><b>Suppression Visites</caption><tr><td >Voulez-vous réellement supprimer ? $ch_input<tr><td style='text-align:center'>
				...
				$form = "<div id='".Utilisateur::$id_div_crud."' ></div><br/>".Utilisateur::bt_ajout(Utilisateur::$bt_class,'Ajouter Utilisateur',false).
				"<form enctype='multipart/form-data' method='post' action='".self::$ficherAction."' target='".self::$ficherIframe."' style=''>".
					"$ch_input".
					"<table>
						<caption style='".self::$style_caption."'><b>".($opt=='ajt'?'Enregistrement':'Modification')." Visites</caption>".
					($opt=='mod'?("<tr>".
					...
					"</tr>"):''). 
					($opt=='mod'?("<tr>".
						"<td><label for='date_heure_visite' >Connecté le</label></td><td>:</td>".
						"<td><input type='text' value=\"$date_heure_visite\" title=\"Connecté le\" placeholder=\"Connecté le\" id='date_heure_visite' name='date_heure_visite' required  class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>"):''). 
					"<tr>".
						"<td><label for='date_heure_deconx' >déConnecté le</label></td><td>:</td>".
						"<td><input type='text' value=\"$date_heure_deconx\" title=\"déConnecté le\" placeholder=\"déConnecté le\" id='date_heure_deconx' name='date_heure_deconx'   class='".self::$input_class."' style='".self::$input_style."' /></td>".
					"</tr>".  
		...
		public static function ini_champ(){
			$T_CH = self::$T_CH = array("id_utilisateur","identite","date_heure_visite","date_heure_deconx");
			$l=0;$ch_alias="c";$label="l";$type="t";
			self::$TLBL = array(
			$T_CH[$l++]=>array($ch_alias=>"sys_visites.id_utilisateur id_utilisateur",$label=>"Utilisateur",$type=>"select"),
			$T_CH[$l++]=>array($ch_alias=>"sys_utilisateur.identite identite",$label=>"Utilisateur",$type=>""),
			$T_CH[$l++]=>array($ch_alias=>"sys_visites.date_heure_visite date_heure_visite",$label=>"Connecté le",$type=>"text"),
			$T_CH[$l++]=>array($ch_alias=>"sys_visites.date_heure_deconx date_heure_deconx",$label=>"déConnecté le",$type=>"text"));
		...
		public static function bt_rapport($class,$val){
			if(!self::$readOnly){echo "
				<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
					<input type='submit' class='$class' id='bt_rapport_complet_".self::$table."' name='bt_rapport_complet_".self::$table."' value='".($val!=''?$val:'Rapport Visites')."' onClick=\"".self::codeOnCallRapport()."\"/>
					".self::$marqueur."
				</form>";
			}
		}
		public static function rapport() {
			...
			$req = "SELECT sys_visites.id $CONSTR[Ch_Select] FROM ".self::$table ." sys_visites 
				INNER JOIN sys_utilisateur sys_utilisateur ON  sys_visites.id_utilisateur = sys_utilisateur.id  ";
		...
		public static function form_rech_rapport(){
			...
			$T_SGO = self::$T_SGO;
			echo"
			<form method='POST' action='".self::$ficherAction."' target='".self::$ficherIframe."'>".(!(isset(self::$CHAMPS['id_utilisateur'][$T_SGO[3]])and self::$CHAMPS['id_utilisateur'][$T_SGO[3]])?"":"<select title=\"Utilisateur\" id='sys_visites_id_utilisateur' name='sys_visites_id_utilisateur'  class='".self::$input_class."'><option value=''>Choisir</option>".Utilisateur::options('',0)."</select>").
				...
				(!(isset(self::$CHAMPS['date_heure_visite'][$T_SGO[3]])and self::$CHAMPS['date_heure_visite'][$T_SGO[3]])?"":"<input type='text' name='sys_visites_date_heure_visite' title=\"Connecté le\" placeholder=\"Connecté le\" class='".self::$input_class."' />").
				(!(isset(self::$CHAMPS['date_heure_deconx'][$T_SGO[3]])and self::$CHAMPS['date_heure_deconx'][$T_SGO[3]])?"":"<input type='text' name='sys_visites_date_heure_deconx' title=\"déConnecté le\" placeholder=\"déConnecté le\" class='".self::$input_class."' />")
		...
		public static function bt_rapport_config($class,$val){
			echo "
			<form method='POST' id='' action='".self::$ficherAction."' target='".self::$ficherIframe."' style='display:inline-block' >
				<input type='submit' class='$class' id='bt_rapport_config_".self::$table."' name='bt_rapport_config_".self::$table."' value='".($val!=''?$val:'Rapport Config Visites')."' onClick=\"".self::codeOnCallConfig()."\"/>
				".self::$marqueur."
			</form>";
		}		
